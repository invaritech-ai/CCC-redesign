    ...
    return (
        ...
        >
            ...
            <div className="relative z-10">
                ...
                {title && (
                    <div className="mr-2 text-lg font-bold leading-tight tracking-wide text-foreground">
                        {title}
                    </div>
                )}
            </div>
...
const HoverRevealSlip = ({ show }: { show: React.ReactNode }) => {
    ...
    return (
        <div className={cn("group relative h-60 w-52 [perspective:1000px]")}>
            ...
            <div
                className={cn(
                    "absolute inset-0 h-full w-48 rounded-lg bg-card shadow-md"
                )}
            ></div>
            ...
            >
                ...
                <div
                    className={cn(
                        "h-full w-full rounded-lg bg-card shadow-md",
                        common
                    )}
                >
                    {show}
            ...
            <div
                className={cn(
                    "z-1 absolute bottom-0 right-0 flex h-48 w-14 -translate-x-10 transform items-start justify-start rounded-r-lg bg-success pl-2 pt-2 text-xs font-bold text-success-foreground transition-transform duration-300 ease-in-out [backface-visibility:hidden] group-hover:translate-x-0 group-hover:rotate-[5deg]"
                )}
            >
